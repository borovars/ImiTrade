...
- **Stock price history** (endpoint returns OHLCV candles from MOEX ISS, not persisted; the frontend renders a linear close-price chart from the `close` field — see `frontend/src/features/stock-details/ui/StockPriceChart.tsx`, a thin wrapper over the shared `shared/components/charts/PriceLineChart.tsx` primitive; **mouse-wheel zooms around the cursor**, LMB-drag lazy-loads older data **same interval**, and a custom DOM-overlay shows a black dashed vertical guide + a brand-color marker + a date/price tooltip on hover) ✓
- **Portfolio value history** (`GET /api/v1/portfolio/history?period=` — `PortfolioService.getHistory` reconstructs the time series of the user's portfolio market value, `Σ quantity_held × close`, by replaying transactions against historical MOEX closes fetched through the **same** `StockHistoryService` — no second market-data source, one MOEX call per distinct ticker per reconstruction; never persisted, in-memory only; empty list for a user with no trades; the frontend renders it via the same `PriceLineChart` primitive with `lazyLoad=false`, period buttons only) ✓
...
├── portfolio/                   # Holdings read model + portfolio value history
│   ├── web/PortfolioController.java  # GET /portfolio, GET /portfolio/history
│   ├── domain/PortfolioService.java  # getPortfolio (live PnL) + getHistory (reconstructed value series)
│   ├── domain/PortfolioPosition.java, PortfolioPositionRepository.java
│   └── dto/PortfolioResponse.java, PortfolioHistoryResponse.java
...
- Test suites:
    ...
    - **portfolio**: `PortfolioPositionRepositoryTest`, `PortfolioServiceTest`, `PortfolioHistoryServiceTest` (history reconstruction — empty/single/multi-stock, multiple buys, partial/full sell, scale-4 rounding, per-ticker MOEX caching), `PortfolioControllerTest`, `PortfolioIntegrationTest`, `PortfolioSecurityTest`
...
- **Portfolio value history**: `GET /api/v1/portfolio/history?period=1D|1W|1M|1Y` (`PortfolioService.getHistory`) reconstructs the time series of the user's **investment portfolio market value** (`Σ quantity_held × close`), mirroring the live `portfolioValue` aggregate of `AccountService` but evaluated at past candle timestamps. **Cash balance is intentionally excluded** — this is the market value of held positions only. Algorithm: load the user's transactions in chronological order (`TransactionRepository.findByUserIdOrderByCreatedAtAsc`) — empty list → return `[]`; resolve distinct traded stock ids → tickers in one batch; fetch each ticker's candle history through `StockHistoryService.getHistory(ticker, period, from)` **once** (local cache keyed by ticker — never re-query MOEX for the same ticker within one reconstruction, avoiding N×M calls); for each union candle timestamp, sum `qty_held(stock, t) × close(stock, t)` across stocks (close forward-filled via `NavigableMap.headMap(t, true).lastEntry()`; stocks with no candle yet or zero held quantity are skipped); points where the user held nothing (before the first buy / after a full sell-out) are skipped to avoid a flat zero segment. Quantities come from a per-stock prefix-sum timeline replayed from `Transaction` rows (BUY adds, SELL subtracts). The result is rounded to money scale 4 (`HALF_UP`), **never persisted** (in-memory computation, same philosophy as `AccountService`/`StockHistoryService`), and MOEX/network failures propagate as `MarketDataUnavailableException` (503) via the existing `GlobalExceptionHandler` — no new error code or handler. No schema migration. The endpoint lives in `PortfolioController` alongside `GET /portfolio` (same `/api/v1/portfolio/**` security rule, JWT-or-Guest). The frontend renders it via the shared `PriceLineChart` primitive with `lazyLoad=false` (period buttons only — backend returns a ready series per period, no scroll-to-past lazy load).
