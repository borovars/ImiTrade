...
- `ui/StockDetailsView.tsx` — контент страницы: Header (компания, тикер-Chip,
  ...
  Information (ticker, companyName, exchange, lotSize — только поля backend).
...
| `stock-details` | ✓ `/stocks/:ticker` — Header, Price Block, User Position (с «N lots»), Trading (reuse диалогов), Company Information (включая Lot Size) |
