...
- `ui/StockPriceChart.tsx` (+ `ui/chart/chartTheme.ts`, `ui/chart/chartZoom.ts`)
  ...
  `CANONICAL_PERIODS` в `chartZoom`); активная кнопка — фирменного цвета.
  **Колесо мыши = зум** относительно точки под курсором (встроенный
  `handleScale.mouseWheel=true`): масштабирует логическую шкалу времени так, что
  точка под курсором остаётся визуальным центром (как в торговых терминалах),
  страница вне графика при этом скроллится как обычно (библиотека сама делает
  `preventDefault` только внутри холста). `handleScroll.mouseWheel` оставлен
  `false` — колесо именно зумит, а не панорамирует. **Перетаскивание ЛКМ**
  (`handleScroll.pressedMouseMove=true`) панорамирует и при выходе за левый край
  догружает более старые данные **тем же интервалом** — серия всегда одного bucket
  size, без артефактов (пиков/провалов). Правый край прибит (`fixRightEdge`), левый
  НЕ прибит (`fixLeftEdge=false`) — выход за загруженные данные триггерит
  lazy-подгрузку (`subscribeVisibleLogicalRangeChange` → `chartZoom.needsLazyLoad` +
  `requestLazyLoad`). **Crosshair-интерфейс при наведении** — собственный DOM-overlay
  ...
  (`vLineStyle`, `1px dashed #000`) через весь холст по X точки данных
  (`timeScale().timeToCoordinate`), **круглый маркер** (`markerStyle`, `#3dba8d` +
  мягкое свечение) в точке пересечения линии графика и направляющей
  (`series.priceToCoordinate(value)` для Y), и **tooltip** с датой и ценой на
  фиксированном смещении справа-снизу от курсора (`positionTooltip`,
  `TOOLTIP_OFFSET_PX`). Появление/исчезновение маркера — плавная CSS-анимация
  (класс `.cct-marker-visible`, инъекция стилей в `ensureOverlayStyle`). Встроенный
  crosshair-маркер серии отключён (`crosshairMarkerVisible: false` в
  `areaSeriesOptions`), чтобы при зуме не появлялась вторая «точка». При зуме (мышь
  неподвижна, `mousemove` не стреляет) overlay перепозиционируется по последней
  позиции курсора (`lastCursorX`) в обработчике `onRangeChange`. Предохранители
  против петель: `loadingRef`, `oldestAvailableDate`, `from >= loadedFrom`.
  **Lazy-подгрузка** ведёт leading-edge `chartZoom.throttle` (220 мс) — запрос уходит
  на первом импульсе, а не ждёт тишины (как debounce); при сильном отдалении чанки
  подгружаются циклом автодогрузки в `requestLazyLoad` (до `MAX_AUTOPRELOAD_CHAIN=4`
  за заход, проверка `needsLazyLoad` после каждого merge), `PRELOAD_FRACTION=0.75`.
  Компонент монтируется с `key={ticker}`, поэтому графики разных акций полностью
  независимы. Контейнер графика всегда в DOM (состояния loading/error/empty —
  overlay поверх canvas), чтобы `createChart` не попадал в цикл ожидания.
...
- `shared/utils/` — чистые функции: `formatMoney` (денежный формат + символ `ᛔ`),
  ...
  денег/дат/PnL/процентов — локальные копии в фичах запрещены.
...
| `stock-details` | ✓ `/stocks/:ticker` — Header (логотип + сектор), Price Block, User Position (с «N lots»), Trading (reuse диалогов), **Price Chart** (линейный график close на lightweight-charts, цвет `#3dba8d`; кнопки 1D/1W/1M/1Y = интервал свечи день/неделя/месяц/квартал; **колесо = зум относительно курсора**, ЛКМ-драг — бесконечный lazy-скролл в прошлое тем же интервалом; crosshair-overlay: чёрная пунктирная направляющая + маркер + tooltip), About (описание), Company Information (Ticker, Name, Exchange, Lot Size, Sector, Website) |
| `trading` | ✓ buy/sell **в лотах** через mutation + диалоги (поле Lots, подсказка `1 lot = N shares`, live-расчёт shares и total) |
| `portfolio` | ✓ `GET /portfolio`, таблица позиций (Quantity с «N lots») + Sell; `DashboardPortfolioPanel` — read-only обзор на Dashboard |
