...
- **`useRegisterMutation`** — то же, но в `register` автоматически прокидывается
  ...
  портфеля и истории; backend начисляет +400 000); после успеха guest-токен
...
- **`endpoints.ts`** — `API_ENDPOINTS` (все пути `/api/v1/...` в одном месте, включая
  `AUTH.LOGIN`/`AUTH.REGISTER`, `USERS.ME`, `TRADING.BUY`/`TRADING.SELL`). Новые пути
...
- `model/useBuyStockMutation.ts`, `model/useSellStockMutation.ts` — `useMutation` с
  ...
  чтобы обновить и каталог, и Stock Detail по тикеру) и тостами.
...
- `ui/StockDetailsView.tsx` — контент страницы: Header (компания, тикер-Chip,
  биржа-Chip), Price Block (текущая цена крупным шрифтом через `formatMoney`),
  User Position (через `usePortfolioQuery` с локальной фильтрацией по `stockId`;
  количество, средняя цена, текущая стоимость, PnL через `formatProfitLoss`;
  если позиции нет — «You do not own this stock.»), Trading (кнопки Buy/Sell,
  открывающие **переиспользованные** `BuyStockDialog`/`SellStockDialog` из
  `features/trading/ui` — без дублирования логики торговли), Company
  Information (ticker, companyName, exchange — только поля backend).
...
- **Backend не меняется**: новых эндпоинтов нет, контракт `StockResponse`
  переиспользуется.
...
- **Подготовка к следующему этапу**: фича изолирует страницу акции; будущие
  график цены (MOEX candles) и история котировок добавятся сюда же
  (вероятно, новыми query-хуками и UI-блоками в `StockDetailsView`).
...
| `stock-details` | ✓ `/stocks/:ticker` — Header, Price Block, User Position, Trading (reuse диалогов), Company Information |
