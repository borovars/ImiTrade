...
| `stocks` | Каталог акций: ticker, company_name, exchange, current_price, lot_size |
...
- **Stock Detail**: детальная информация об акции на маршруте `/stocks/:ticker`
  ...
  `SellStockDialog` из Trading), Company Information (включая Lot Size).
  Данные акции берутся фильтром `GET /api/v1/stocks?ticker=<ticker>`
  ...
  Подготавливает почву для интеграции графика цены (MOEX candles) и истории
  котировок на следующих этапах.
...
- `MoexClient.getMarketSnapshot` запрашивает для тикера последнее значение цены
  ...
  блоке `securities`, а не в `marketdata`.
