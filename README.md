...
- **Исторический график цены**: профессиональный интерактивный **линейный** график
  ...
  **Lightweight Charts** (TradingView). Линия цвета `#3dba8d` с прозрачным градиентом,
  ...
  1Y — квартальные (−10 лет). Перетаскивание ЛКМ и колесо мыши догружают более
  старые данные **тем же интервалом** (бесконечный скролл в прошлое, 1D→+неделя,
  1W→+2 месяца, 1M→+год, 1Y→+2 года за щелчок колеса), вправо в будущее двигать нельзя.
  Lazy loading с защитой от бесконечных запросов. Данные —
...
Lightweight Charts 5 (линейный график цены акции, только close; OHLCV отсекается на фронте).
...
- **Stocks**: каталог акций (`GET /api/v1/stocks`) с текущей ценой, размером лота
  (колонка Lot Size) и кнопками Buy/Sell в каждой строке. Тикер в строке —
  ссылка на страницу детали акции.
- **Stock Detail**: детальная информация об акции на маршруте `/stocks/:ticker`
  ...
  **Lightweight Charts**, данные — `GET /api/v1/stocks/{ticker}/history?period=&from=`
  ...
  3 года / 10 лет; активная кнопка подсвечена фирменным цветом `#3dba8d`. Перетаскивание
  ЛКМ и колесо мыши догружают более старые данные **тем же интервалом** — серия всегда
  одного bucket size, без артефактов; один щелчок колеса = +шаг в прошлое (1D→+неделя,
  1W→+2 месяца, 1M→+год, 1Y→+2 года). Колесо зарегистрировано как **non-passive**
  listener, чтобы `preventDefault` блокировал скролл страницы. Правый край прибит к
  последней свече (`fixRightEdge`) — в будущее двигать нельзя. Lazy-подгрузка с
  дебаунсом и предохранителями против бесконечных запросов; достижение
  `oldestAvailableDate` (пустой ответ) запрещает дальнейший скролл влево. График
  автоматически масштабируется (`autoSize` через внутренний `ResizeObserver`). Tooltip
  с датой и ценой — через `subscribeCrosshairMove` (пишется в DOM напрямую, без
  re-render). Состояния Loading (Skeleton), Error («Не удалось загрузить историю цены»
  + Retry), Empty («Нет данных за период») переиспользуют общие компоненты. История не
  персистится и кэшируется React Query (ключ `queryKeys.stockHistory(ticker, period, from)`).
- **Trading**: покупка и продажа акций **в лотах**
  ...
  N shares») и estimated total. Backend сам вычисляет `quantity = lots × lotSize`;
...
- **История цены (candles)**: `MoexHistoryClient` (в `stocks/integration/moex/`)
  ...
  диапазон дат и интервал (`interval`) по периоду (`HistoryPeriod`: 1D→10 мин,
  1W/1M→1 ч, 3M/1Y→1 день), `MoexHistoryMapper` преобразует строки в `CandleResponse`
